...
const ПІДПИС_ЧУЖОЇ = /по\s+роках|за\s+роками|динамік\S*\s+цін|архів|курс\S*\s+валют/i;
...
function підписТаблиці(текстПеред, тіло) {
  ...
  // ⚠️ Теги міняємо на ПРОБІЛ і ріжемо ВЖЕ ОЧИЩЕНИЙ текст. Інакше дві біди,
  // обидві перевірені прогоном: cleanText вирізає теги порожнім рядком, тож
  // «<span>Середні</span><span>ціни</span>» склеювалося в «Середніціни» і
  // підпис не впізнавався (ОККО виходила 45 замість 60); а вікно в 400 знаків,
  // відрізане по сирому HTML, з'їдали довгі теги.
  const словами = s => cleanText(String(s).replace(/<[^>]+>/g, ' '));
  return словами(текстПеред).slice(-400) + ' ' + словами(c ? c[1] : '');
}
